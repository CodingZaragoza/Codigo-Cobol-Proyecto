...
/**
 * Applies add-on costs for GoreTag if the technology was indicated in the claim.
 *
 * @since 2023
 */
public class CalculateGoreTagAddOnCost extends CalculateTechnologyAddOnCost {
  ...
  public void calculate(IppsPricerContext calculationContext) {
    ...
    if (dataTables.codesMatch(
            "GORETAG1", ClaimCodeType.PROC, calculationContext.getClaimData().getProcedureCodes())
        && dataTables.codesMatch(
            "GORETAG2",
            ClaimCodeType.PROC,
            calculationContext.getClaimData().getProcedureCodes())) {
      aggregateNewTechCost(calculationContext, dataTables.getNewTechnologyAmount("GORETAG"));
    }
  }
}
